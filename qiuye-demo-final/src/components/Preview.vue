<template>
    <div class="mykit-preview">
      ...
      <div class="preview-bottom">
        <span name="Code" @click="showSourceCode">{{showSourceCodetext ? '显示代码' : '隐藏代码'}}</span>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    async mounted() {
      if (this.compName && this.demoName) {
        if (isDev) {
          ...
          this.sourceCode = await fetch(`${isDev ? '' : '/qiuye-demo-final'}/packages/components/${this.compName}/docs/${this.demoName}.vue`).then((res) => res.text());
        }
      }
      await this.$nextTick();
      Prism.highlightAll();
    ...
    methods: {
      ...
      showSourceCode() {
        ...
        this.showSourceCodetext = !this.showSourceCodetext
      },
    },
  };
  </script>
